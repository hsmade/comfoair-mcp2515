...
class Comfoair : public mcp2515::MCP2515 {
public:
    Comfoair(): public MCP2515() { }

    void Comfoair::setup(){
        this->set_mcp_clock(MCP_8MHZ);
        this->set_mcp_mode(CANCTRL_REQOP_NORMAL);
        this->set_bitrate_(CAN_50KBPS, MCP_8MHZ);
...
//        CAN0.watchFor();
    ...
    void Comfoair::loop(){
        // ESP_LOGD(TAG, "loop");
        if (this->read_message(canMessage)) {
